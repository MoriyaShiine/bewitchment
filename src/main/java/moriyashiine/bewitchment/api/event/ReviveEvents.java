...
public final class ReviveEvents {
    public static final Event<OnRevive> ON_REVIVE = createArrayBacked(OnRevive.class,
            listeners -> (player, source, poppet) -> {
    ...
    public static final Event<CancelRevive> CANCEL_REVIVE = createArrayBacked(CancelRevive.class,
            listeners -> (player, source, poppet) -> {
                for (CancelRevive listener : listeners) {
                    if (listener.shouldCancel(player, source, poppet)) return true;
                }
                return false;
            }
    );
