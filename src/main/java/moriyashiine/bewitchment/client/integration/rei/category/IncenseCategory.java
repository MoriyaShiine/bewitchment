...
public class IncenseCategory implements RecipeCategory<IncenseCategory.Display> {
	public static final Identifier IDENTIFIER = new Identifier(Bewitchment.MODID, "incenses");
	...
	public static class Display implements RecipeDisplay {
		...
		public Display(IncenseRecipe recipe) {
			...
			this.input = input;
			output = Collections.singletonList(Collections.singletonList(EntryStack.create(PotionUtil.setCustomPotionEffects(new ItemStack(BWObjects.BRAZIER), Collections.singletonList(new StatusEffectInstance(recipe.effect, 24000, recipe.amplifier))))));
		}
