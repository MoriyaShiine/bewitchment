...
public class BrazierBlockEntityRenderer extends BlockEntityRenderer<BrazierBlockEntity> {
	public BrazierBlockEntityRenderer(BlockEntityRenderDispatcher dispatcher) {
		super(dispatcher);
	}
		...
		if (world != null) {
			...
			if (!one.isEmpty()) {
				...
				if (!two.isEmpty()) {
					...
					MinecraftClient.getInstance().getItemRenderer().renderItem(two, ModelTransformation.Mode.FIXED, light, overlay, matrices, vertexConsumers);
					matrices.pop();
					...
					if (!three.isEmpty()) {
						...
						MinecraftClient.getInstance().getItemRenderer().renderItem(three, ModelTransformation.Mode.FIXED, light, overlay, matrices, vertexConsumers);
						matrices.pop();
						...
						if (!four.isEmpty()) {
							...
							MinecraftClient.getInstance().getItemRenderer().renderItem(four, ModelTransformation.Mode.FIXED, light, overlay, matrices, vertexConsumers);
							matrices.pop();
						}
					}
				}
