...
public class MakeDayRitualFunction extends RitualFunction {
	@Override
	...
	public boolean isValid(World world, BlockPos pos) {
		return world.isNight() && world.getLevelProperties().getGameRules().getBoolean(GameRules.DO_DAYLIGHT_CYCLE);
	}
	
	public void runningTick(Ritual ritual, World world, BlockPos pos) {
		if (world.isClient) {
			world.addParticle(ParticleTypes.FLAME, true, pos.getX() + 0.5 + MathHelper.nextFloat(world.random, -0.2f, 0.2f), pos.getY() + 0.5 + MathHelper.nextFloat(world.random, -0.2f, 0.2f), pos.getZ() + 0.5 + MathHelper.nextFloat(world.random, -0.2f, 0.2f), 0, 0, 0);
		}
	}
