...
public abstract class BWTameableEntity extends TameableEntity {
	...
	public void readCustomDataFromTag(CompoundTag tag) {
		...
		if (getVariants() > 1) {
			dataTracker.set(VARIANT, tag.getByte("Variant"));
		}
	}
		...
		if (variants > 1) {
			if (hasShiny()) {
				if (random.nextInt(8192) == 0) {
					dataTracker.set(VARIANT, (byte) 0);
				}
				else {
					dataTracker.set(VARIANT, (byte) (random.nextInt(variants - 1) + 1));
				}
			}
			else {
	...
	public abstract byte getVariants();
