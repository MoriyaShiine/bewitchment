...
public class DemonTrade {
	...
	public static class Loader extends JsonDataLoader {
		...
		private static final Gson GSON = LootGsons.getFunctionGsonBuilder().registerTypeAdapter(DemonTrade.class, new DemonTrade.Serializer()).create();
		
		public Loader() {
			super(GSON, "demon_trades");
		}
	...
	public static class Serializer implements JsonDeserializer<DemonTrade> {
		...
		public DemonTrade deserialize(JsonElement element, Type typeOfT, JsonDeserializationContext context) throws JsonParseException {
			...
			return new DemonTrade(deserializeSupplier(context, JsonHelper.getObject(json, "first_buy")), json.has("second_buy") ? deserializeSupplier(context, JsonHelper.getObject(json, "second_buy")) : (lootContext) -> ItemStack.EMPTY, deserializeSupplier(context, JsonHelper.getObject(json, "sell")), JsonHelper.getInt(json, "max_uses", 10), JsonHelper.getFloat(json, "chance", 1F), JsonHelper.getFloat(json, "luck_bonus", 0.5F));
		}
